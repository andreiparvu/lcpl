  ...
  (s=statement {
    ...
    if (body.get(0).size() == 0) {
      if ($s.result instanceof LocalDefinition) {
        line = $s.localLine; // dorim linia unde a aparut 'local'
      } else {
...
local returns [LocalDefinition def, LocalDefinition defEnd, int line]
  ...
  ^(LOCAL
    (
      (^(DECL id1=id id2=id) {
        decls.add(
          new LocalDefinition($id1.line, $id2.text, $id1.text, null, null)
        );
      })
      |
      (^(DECL id1=id id2=id e=expr) {
        decls.add(
          new LocalDefinition($id1.line, $id2.text, $id1.text, $e.expr, null)
        );
      })
    )*
  ) {
