...
public class Assignment extends Expression {
  ...
  public String emitCode(PrintStream os, boolean isValue) {
    // Incarcam simbolul ca referinta
    String symbolIndex = symbolData.load(os, false);

    // Incarcam expresia din dreapta ca valoare
    String e1Index = e1.emitCode(os, true);

    os.println(
        Properties.store(Properties.genType(e1.getTypeData().getName()), e1Index, symbolIndex)
        );
